...
@implementation LSPDownloadManager

+ (instancetype)sharedInstance {
    static LSPDownloadManager *manager = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        manager = [LSPDownloadManager new];
    });
    return manager;
}
...
- (instancetype)init {
    if (self = [super init]) {
        ...
        //        [toDownloadArr enumerateObjectsUsingBlock:^(LSPDownloadModel *obj, NSUInteger idx, BOOL * _Nonnull stop) {
        //            if (obj.state == LSPDownloadModelStatePaused) {
        //                obj.state = LSPDownloadModelStateWaiting;
        //            }
        //
...
- (void)addDownloadArr:(NSArray<LSPDownloadModel *> *)downloadArr {
    ...
    // 新加入的模型，需要把状态改为等待下载中
    [downloadArr enumerateObjectsUsingBlock:^(LSPDownloadModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if (obj.state != LSPDownloadManagerStateFinished && obj.state != LSPDownloadManagerStateDownloading) {
            obj.state = LSPDownloadManagerStateWaiting;
            
            [self updateDownloadModel:obj];
            
            if ([self.delegate respondsToSelector:@selector(LSPDownloadManager:downloadModel:stateChanged:)]) {
                [self.delegate LSPDownloadManager:self downloadModel:obj stateChanged:LSPDownloadManagerStateWaiting];
            }
        }
    }];
...
- (void)removeDownloadArr:(NSArray<LSPDownloadModel *> *)downloadArr {
    ...
    if ([self.delegate respondsToSelector:@selector(LSPDownloadManager:removedDownloadArr:)]) {
        [self.delegate LSPDownloadManager:self removedDownloadArr:downloadArr];
    }
...
- (void)pausedDownloadArr:(NSArray<LSPDownloadModel *> *)downloadArr {
    ...
    [downloadList enumerateObjectsUsingBlock:^(LSPDownloadModel * _Nonnull model, NSUInteger idx, BOOL * _Nonnull stop) {
        [downloadArr enumerateObjectsUsingBlock:^(LSPDownloadModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            // 文件未下载完成，并且是需要暂停的文件
            if ([model.fileID isEqualToString:obj.fileID] && model.state != LSPDownloadManagerStateFinished) {
                model.state = LSPDownloadManagerStatePaused;
                
                [self updateDownloadModel:model];
                
                if ([self.delegate respondsToSelector:@selector(LSPDownloadManager:downloadModel:stateChanged:)]) {
                    [self.delegate LSPDownloadManager:self downloadModel:model stateChanged:LSPDownloadManagerStatePaused];
                }
            }
        }];
    }];
...
- (void)resumeDownloadArr:(NSArray<LSPDownloadModel *> *)downloadArr {
    ...
    [downloadList enumerateObjectsUsingBlock:^(LSPDownloadModel * _Nonnull model, NSUInteger idx, BOOL * _Nonnull stop) {
        [downloadArr enumerateObjectsUsingBlock:^(LSPDownloadModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            // 文件未下载完成并且是需要恢复下载的文件
            if ([model.fileID isEqualToString:obj.fileID] && model.state != LSPDownloadManagerStateFinished) {
                model.state = LSPDownloadManagerStateWaiting;
                
                [self updateDownloadModel:model];
                
                if ([self.delegate respondsToSelector:@selector(LSPDownloadManager:downloadModel:stateChanged:)]) {
                    [self.delegate LSPDownloadManager:self downloadModel:model stateChanged:LSPDownloadManagerStateWaiting];
                }
            }
        }];
    }];
...
- (void)clearDownload {
    ...
    [[self downloadFileList] enumerateObjectsUsingBlock:^(LSPDownloadModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if (obj.state != LSPDownloadManagerStateFinished) {
    ...
    if ([self.delegate respondsToSelector:@selector(LSPDownloadManager:removedDownloadArr:)]) {
        [self.delegate LSPDownloadManager:self removedDownloadArr:toClearArr];
    }
}
...
- (void)startDownloadTask {
    ...
    if ((self.downloadTask && self.downloadTask.state == NSURLSessionTaskStateRunning) || (self.dataTask && self.dataTask.state == NSURLSessionTaskStateRunning)) {
        ...
        __block LSPDownloadModel *model = nil;
        [downloadList enumerateObjectsUsingBlock:^(LSPDownloadModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            if (obj.state == LSPDownloadManagerStateWaiting) {
                model = obj;
                *stop = YES;
            }
        }];
        ...
        if (model) {
            ...
            model.state = LSPDownloadManagerStateDownloading;
            
            [self updateDownloadModel:model];
            
            // 下载中
            if ([self.delegate respondsToSelector:@selector(LSPDownloadManager:downloadModel:stateChanged:)]) {
                [self.delegate LSPDownloadManager:self downloadModel:model stateChanged:LSPDownloadManagerStateDownloading];
            }
...
- (void)startDownloadDataWithModel:(LSPDownloadModel *)model {
    ...
    self.downloadTask = [self.manager downloadTaskWithRequest:request progress:^(NSProgress * _Nonnull downloadProgress) {
        ...
        if ([strongSelf.delegate respondsToSelector:@selector(LSPDownloadManager:downloadModel:totalSize:downloadSize:progress:)]) {
            [strongSelf.delegate LSPDownloadManager:strongSelf
                                     downloadModel:model
        ...
        if (error) { // 下载失败
            ...
            model.state = LSPDownloadManagerStateFailed;
            [strongSelf updateDownloadModel:model];
            
            if ([strongSelf.delegate respondsToSelector:@selector(LSPDownloadManager:downloadModel:stateChanged:)]) {
                [strongSelf.delegate LSPDownloadManager:self downloadModel:model stateChanged:LSPDownloadManagerStateFailed];
            }
        }else { // 下载完成
            model.state = LSPDownloadManagerStateFinished;
            
            [strongSelf updateDownloadModel:model];
            
            if ([strongSelf.delegate respondsToSelector:@selector(LSPDownloadManager:downloadModel:stateChanged:)]) {
                [strongSelf.delegate LSPDownloadManager:self downloadModel:model stateChanged:LSPDownloadManagerStateFinished];
            }
            
            if ([strongSelf.delegate respondsToSelector:@selector(LSPDownloadManager:removedDownloadArr:)]) {
                [strongSelf.delegate LSPDownloadManager:strongSelf removedDownloadArr:[NSArray arrayWithObject:model]];
            }
        }
...
- (void)startBreakpointWithModel:(LSPDownloadModel *)model {
    ...
    [self.manager setDataTaskDidReceiveDataBlock:^(NSURLSession * _Nonnull session, NSURLSessionDataTask * _Nonnull dataTask, NSData * _Nonnull data) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            if ([strongSelf.delegate respondsToSelector:@selector(LSPDownloadManager:downloadModel:totalSize:downloadSize:progress:)]) {
                [strongSelf.delegate LSPDownloadManager:strongSelf downloadModel:model totalSize:model.fileLength downloadSize:model.currentLength progress:progress];
            }
        });
    }];
...
// 保存数据模型
- (void)saveDownloadModelArr:(NSArray <LSPDownloadModel *> *)modelArr {
    NSMutableArray *downloadModelArr = nil;
    if ([self ifPathExist:[self downloadDataFilePath]]) {
        downloadModelArr = [self downloadFileList];
        [modelArr enumerateObjectsUsingBlock:^(LSPDownloadModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
// 更新数据模型
- (void)updateDownloadModel:(LSPDownloadModel *)model {
    if ([self ifPathExist:[self downloadDataFilePath]]) {
        NSMutableArray *downloadArr = [self downloadFileList];
        
        [downloadArr enumerateObjectsUsingBlock:^(LSPDownloadModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            if ([model.fileID isEqualToString:obj.fileID]) {
                if (model.state == LSPDownloadManagerStateFinished) {
...
// 删除数据模型
- (void)deleteDownloadModelArr:(NSArray <LSPDownloadModel *> *)modelArr {
    if ([self ifPathExist:[self downloadDataFilePath]]) {
        NSMutableArray *downloadArr = [self downloadFileList];
        
        [modelArr enumerateObjectsUsingBlock:^(LSPDownloadModel * _Nonnull model, NSUInteger idx, BOOL * _Nonnull stop) {
            [[downloadArr mutableCopy] enumerateObjectsUsingBlock:^(LSPDownloadModel   * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                if ([model.fileID isEqualToString:obj.fileID]) {
                    // 状态改变
                    model.state = LSPDownloadManagerStateNone;
                    [self updateDownloadModel:model];
                    
                    if ([self.delegate respondsToSelector:@selector(LSPDownloadManager:downloadModel:stateChanged:)]) {
                        [self.delegate LSPDownloadManager:self downloadModel:model stateChanged:LSPDownloadManagerStateNone];
                    }
...
- (NSMutableArray <LSPDownloadModel *> *)downloadFileList {
    if ([self ifPathExist:[self downloadDataFilePath]]) {
        return [LSPDownloadModel mj_objectArrayWithFile:[self downloadDataFilePath]];
    }
    return nil;
}

// 遍历文件（已下载）
- (NSMutableArray <LSPDownloadModel *> *)downloadedFileList {
    if ([self ifPathExist:[self downloadDataFilePath]]) {
        NSMutableArray *downloadArr = [self downloadFileList];
...
// 遍历文件（未下载、下载中、暂停）
- (NSMutableArray <LSPDownloadModel *> *)toDownloadFileList {
    if ([self ifPathExist:[self downloadDataFilePath]]) {
        NSMutableArray *downloadArr = [self downloadFileList];
...
- (void)writeToFileWithModelArr:(NSArray <LSPDownloadModel *> *)modelArr {
    NSArray *dataArr = [LSPDownloadModel mj_keyValuesArrayWithObjectArray:modelArr];
...
- (BOOL)ifExistDownloadModelWithID:(NSString *)ID {
    __block BOOL exist = NO;
    if ([self ifPathExist:[self downloadDataFilePath]]) {
        NSArray *modelArr = [LSPDownloadModel mj_objectArrayWithFile:[self downloadDataFilePath]];
        [modelArr enumerateObjectsUsingBlock:^(LSPDownloadModel *obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (unsigned long long)fileSizeWithModel:(LSPDownloadModel *)model {
